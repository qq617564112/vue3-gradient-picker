	...
	<div class="vue-gpickr">
		...
		<div class="vue-gpickr-inner-container">
			...
			<div class="vue-gpickr-stops-container" ref="stopsContainer">
				...
				<div class="vue-gpickr-stop" v-for="(stop, index) in stops" :key="index" :style="stopStyle(index)" :class="{ active: index == currentStopIdx }" @mousedown.stop="handleMouseDown(index, $event)"
					@touchstart.stop="handleTouchstart(index, $event)" @contextmenu.stop="handleMouseRightCLick(index,$event)">
				</div>
			</div>
...
export default defineComponent({
	...
	methods: {
		emitInput(angle, stops) {
			this.$emit("update:modelValue", new LinearGradient({ angle, stops }));
		},
		...
		addStop(event) {
			const position =
				Math.round((event.offsetX * 100) / event.target.offsetWidth) / 100;
		...
		handleChange(event) {
			...
			if (this.stops.length > 2) {
				...
				const y =
					(event.touches ? event.touches[0].clientY : event.clientY) || 0;
				const verticalDistance = Math.abs(
					y - this.containerBoundingClientRectangle.bottom
				);
